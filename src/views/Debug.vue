<template>
  <div>
    <v-btn @click="OpenPort" color="primary" class="ma-2"
      >Open bluetootch port</v-btn
    >
    <v-btn @click="ClosePort" color="primary" class="ma-2"
      >Close bluetootch port</v-btn
    >
    ...
    <v-btn @click="getFirmwareVersion" color="primary" class="ma-2"
      >get firmware version</v-btn
    >
    <v-btn @click="getStatus" color="primary" class="ma-2">get status</v-btn>
    <v-btn @click="bluetooth_isConnected" color="primary" class="ma-2"
      >bluetooth is connected</v-btn
    >
    <v-btn @click="subscribe" color="primary" class="ma-2">subscribe</v-btn>
    <v-btn @click="setListCommand" color="primary" class="ma-2"
      >setListCommand</v-btn
    >
    ...
    Device status {{ getDeviceStatus }} Device timer {{ getDeviceTimer }}
...
export default {
  ...
  methods: {
    ...
    async OpenPort() {
      let res = await this.openBluetoothPort().catch((err) =>
    ...
    async ClosePort() {
      let res = await this.closeBluetoothPort().catch((err) =>
        console.log("error:", err)
      );
    ...
    async readDada() {
      let data = await this.readPort().catch((err) =>
        console.log("read port error", err)
      );
    ...
    setListCommand() {
      this.writePort(
        constans.startByte + "50" + "1000" + "03" + "20" + constans.stopByte
      );
    },
