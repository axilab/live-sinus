<template>
  <div style="margin: 0; padding: 0">
    <v-dialog
      v-if="DialogDevicesSelectShow"
      v-model="DialogDevicesSelectShow"
      fullscreen
      hide-overlay
      scrollable
    >
      <devices-select
        :prevDevice="DialogDevicesSelectData"
        @DialogDevicesSelectCallback="DialogDevicesSelectCallback($event)"
      ></devices-select>
    </v-dialog>
  ...
  computed: {
    ...
    descriptionForButtonSelectGenetator() {
      if (this.deviceSelect == null || this.deviceSelect == undefined) {
        ...
        return (
          this.$t("settings.descriptionForButtonSelectGenetatorSelect") +
          " " +
          this.deviceSelect.name
        );
      }
    },
  },
