<template>
    <v-card>
        ...
        <v-card-actions>
            <v-btn color="primary" @click="clickSelect">{{$t('settings.selectDeviceDialogButtonSelect')}}</v-btn>
            <v-btn color="primary" text @click="clickBack">{{$t('settings.selectDeviceDialogButtonCancel')}}</v-btn>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            clickSelect(){
                const res = this.devices.find((item)=>{if (item.address==this.selectDevice)return true})
