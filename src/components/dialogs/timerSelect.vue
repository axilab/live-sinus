  ...
  <v-card>
    ...
    <v-card-text style="height: 300px">
      <div class="parent_div">
        ...
        ></Scroller>
        <Scroller
          :itemSelect="Number(value.charAt(3))"
          :itemList="vList"
...
export default {
  ...
  methods: {
    ...
    clickSelect() {
      ...
      const hours = Number(
        String(this.currentValue[0]) + String(this.currentValue[1])
      );
      const minut = Number(
        String(this.currentValue[2]) + String(this.currentValue[3])
      );
      const result = hours * 60 + minut;
      this.$emit("Callback", { result: result, type: this.type });
    },
  },
