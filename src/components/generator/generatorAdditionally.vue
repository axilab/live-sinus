<template>
    <div>
        ...
        <v-dialog v-if="Dialognum2selectShow" v-model="Dialognum2selectShow" scrollable>
            <num2select :input="DialogData" @Callback="DialogSelectCallback($event)"></num2select>
        </v-dialog>
        ...
        >
            <v-list-item v-if="amperageStabilizationVisable">
                <template>
                    ...
                    <v-list-item-content @click="selectedAmperageStabilization=!selectedAmperageStabilization">
        ...
        <v-list-item-group color="primary" v-if="frequencyAutotuningVisable">
            <v-list-item @click="clickSetting('main.settingsList.frequency_autotuning')">
                ...
                <v-list-item-content>
                    ...
                    <v-list-item-subtitle>{{frequencyAutotuningValue}}</v-list-item-subtitle>
                </v-list-item-content>
            </v-list-item>
        </v-list-item-group>
    ...
    export default {
        ...
        watch: {
            selectedErrorOff(newValue){
                ...
                if (val!==old){this.setErrorOff(val, false)}
            },
            ...
            selectedAmperageStabilization(newValue){
                ...
                if (val!==old){this.setAmperageStabilization(val, false)}
            ...
            clickSetting(item){
                ...
                switch (item) {
                    case 'main.settingsList.frequency_autotuning':
                        this.DialogData = {value: this.$store.getters.getD77, type:'frequency_autotuning',title:"main.titles.frequency_autotuning"}
        ...
        created() {
            this.selectedErrorOff= (String(this.$store.getters.getD65)==="1") ? true : false
            this.selectedAmperageStabilization = (String(this.$store.getters.getD79)==="1") ? true : false
        }
    }
</script>
