      ...
      <v-list-item-group color="primary">
        ...
        <v-list-item>
          <template>
            ...
            <v-list-item-content
              @click="selectModulationOn = !selectModulationOn"
            >
              <v-list-item-title>{{
                $t("main.settingsList.modulation")
              }}</v-list-item-title>
            </v-list-item-content>
          </template>
        </v-list-item>
...
export default {
  ...
  computed: {
    items() {
      ...
      if (
        this.$store.getters.getD69 === "3" ||
        this.$store.getters.getD69 === "4"
      ) {
        options.push({
          text: "main.settingsList.am_dutyCycle",
          icon: "mdi-cog-outline",
        });
      }

      options.push({
        text: "main.settingsList.am_depth",
        icon: "mdi-cog-outline",
      });
      options.push({
        text: "main.settingsList.am_frequency",
        icon: "mdi-cog-outline",
      });
      options.push({
        text: "main.settingsList.fm_form",
        icon: "mdi-cog-outline",
      });

      if (
        this.$store.getters.getD73 === "3" ||
        this.$store.getters.getD73 === "4"
      ) {
        options.push({
          text: "main.settingsList.fm_dutyCycle",
          icon: "mdi-cog-outline",
        });
      }
      options.push({
        text: "main.settingsList.fm_deviation",
        icon: "mdi-cog-outline",
      });
      options.push({
        text: "main.settingsList.fm_frequency",
        icon: "mdi-cog-outline",
      });
      return options;
    },
  ...
  watch: {
    ...
    selectModulationOn(newValue) {
      ...
      if (val !== old) {
        this.setGeneratorModulation(val, false);
      }
    },
  ...
  methods: {
    ...
    clickSetting(item) {
      ...
      switch (item) {
        ...
        case "main.settingsList.am_frequency":
          this.DialogData = {
            value: this.$store.getters.getD68,
            type: "main.titles.am_frequency",
        ...
        case "main.settingsList.fm_deviation":
          this.DialogData = {
            value: this.$store.getters.getD71,
            type: "main.titles.fm_deviation",
        ...
        case "main.settingsList.fm_frequency":
          this.DialogData = {
            value: this.$store.getters.getD72,
            type: "main.titles.fm_frequency",
    ...
    subtitle(param) {
      switch (param) {
        case "main.settingsList.am_form":
          return this.$t(
            constans.waveWormFromIndex[this.$store.getters.getD69]
          );
        case "main.settingsList.am_dutyCycle":
        ...
        case "main.settingsList.fm_form":
          return this.$t(
            constans.waveWormFromIndex[this.$store.getters.getD73]
          );
        case "main.settingsList.fm_dutyCycle":
